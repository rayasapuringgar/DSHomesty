...
if ( ! function_exists( 'grandium_breadcrubms' ) ) {
    function grandium_breadcrubms() {
        global $wp_query, $post, $paged;

        $space      = ' ';
        $on_front   = get_option( 'show_on_front' );
        $blog_page  = get_option( 'page_for_posts' );
        $separator  = $space . '<li class="bredcrumb-separator">' . apply_filters( 'grandium_breadcrumb_separator',
		'' ) . '</li>' . $space;
        $link       = apply_filters( 'grandium_breadcrumb_link', '<li><a href="%1$s" title="%2$s" rel="bookmark" class="breadcrumb-item">%2$s</a></li>' );
        $current    = apply_filters( 'grandium_breadcrumb_current', '<li class="breadcrumb-current">%s</li>' );
        ...
        if ( $on_front == "page" && is_home() ) {
            $blog_title = isset( $blog_page ) ? get_the_title( $blog_page ) : esc_html__( 'Our Blog', 'grandium' );
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'grandium' ) ) . $separator . sprintf( $current, $blog_title );
        } else {
            $out .= sprintf( $link, home_url(), esc_html__( 'Home', 'grandium' ) );
        }
        ...
        if ( is_singular() ) {
            ...
            if ( is_post_type_archive() ) {
                ...
                $out .= $separator . esc_html__( 'Category: ', 'grandium' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_tag() ) {
                $out .= $separator . esc_html__( 'Tag: ', 'grandium' ) . sprintf( $current, $wp_query->queried_object->name );
            } else if ( is_date() ) {
                $out .= $separator;
                if ( is_day() ) {
                    global $wp_locale;
                ...
                $out .= $separator . sprintf( $current, esc_html__( 'Error 404', 'grandium' ) );
            } else if ( is_search() ) {
                $out .= $separator . sprintf( $current, esc_html__( 'Search', 'grandium' ) );
            }

        }

        $out .= '';
        echo apply_filters( 'grandium_breadcrumbs_out', $out );
    }
}

add_action( 'grandium_before_template_header', 'grandium_breadcrubms' );